...
```bash
npm run pack     # unsigned Den.app in release/mac-arm64/ (fast, local use)
# npm run dist   # full distributable (dmg) — needs signing config
cp -R release/mac-arm64/Den.app /Applications/   # install it
```

The app icon is generated by `scripts/make-icon.cjs` (Electron renders the 🦊
into `build/icon.png` → `build/icon.icns`). Regenerate with
`electron scripts/make-icon.cjs` then rebuild the iconset.
